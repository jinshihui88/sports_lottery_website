...
public class BettingRecordServiceImpl extends ServiceImpl<BettingRecordMapper, BettingRecord> implements BettingRecordService {
    
    private final BettingRecordMapper bettingRecordMapper;
    
    @Override
    public IPage<BettingRecord> getRecordPage(BettingRecordQuery query) {
        Page<BettingRecord> page = new Page<>(query.getCurrent(), query.getSize());
    ...
    public boolean addRecord(Long userId, BettingRecordRequest request) {
        ...
        record.setBetContent(request.getBetContent());
